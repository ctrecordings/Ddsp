...
public:
    ...
    override void processBuffers(const(T)* inputBuffer, T* outputBuffer, int numSamples)
    {
        foreach (sample; 0 .. numSamples)
        {
            ...
            outputBuffer[sample] = _mix * yn + (1.0 - _mix) * xn;
    ...
    {
        return _feedback * buffer[cast(size_t) _readIndex];
    }

    float getFeedbackAmount() nothrow @nogc
    {
        return _feedback;
    }

    float getMixAmount() nothrow @nogc
    {
        return _mix;
    }

    void setCurrentFeedbackInput(float f) nothrow @nogc
    {
        _feedbackIn = f;
    }

    void setUseExternalFeedback(bool b) nothrow @nogc
    {
        _useExternalFeedback = b;
    }

    void addFeedbackEffect(AudioEffect!T effect) nothrow @nogc
    {
        feedbackFX.pushBack(effect);
    }

    void setFeedbackAmount(float feedback) nothrow @nogc
    {
        _feedback = feedback;
    }

    void setMixAmount(float mix) nothrow @nogc
    {
        _mix = mix;
    }
    ...
    override string toString()
    {
        ...
        string output = "Rindex " ~ to!string(_readIndex) ~ " Windex " ~ to!string(
            _writeIndex) ~ " Size " ~ to!string(_size) ~ " Delay " ~ to!string(_delayInSamples);
        return output;
    }
